# ADR-0007: `show` — human overview and `--json`
...
- **`show` reuses the same strict read as `add`.** The strict-read logic behind
  ...
  into a shared `LoadError` and a `load_manifest` that returns both the manifest's
  exact file bytes and the parsed `Manifest`. `add` maps `LoadError` into its own
  `AddError::Load`; `show` surfaces it directly. So `show` never
...
- `AddError` no longer owns the read-path variants; they live on `LoadError` and
  reach `AddError` through `AddError::Load`. The CLI's error text is unchanged
